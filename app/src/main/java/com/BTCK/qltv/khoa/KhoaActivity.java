...
public class KhoaActivity extends AppCompatActivity {
    ...
    private void filterData(String keyword) {
        ...
        for (Khoa khoa : listGoc) {
            String tenKhoa = khoa.getTenKhoa() == null ? "" : khoa.getTenKhoa();
            if (tuKhoa.isEmpty() || tenKhoa.toLowerCase(Locale.getDefault()).contains(tuKhoa)) {
                listHienThi.add(khoa);
                listHienThiString.add(tenKhoa + " (" + khoa.getMaKhoa() + ")");
            }
        }
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        Khoa khoaEdit = listHienThi.get(selectedPosition);

        if (item.getItemId() == R.id.menu_update) {
            Intent intent = new Intent(KhoaActivity.this, UpdateKhoaActivity.class);
            intent.putExtra("maKhoa", khoaEdit.getMaKhoa());
            intent.putExtra("tenKhoa", khoaEdit.getTenKhoa());
            startActivity(intent);

        } else if (item.getItemId() == R.id.menu_delete) {
            new AlertDialog.Builder(this)
                    .setTitle("Xác nhận xóa")
                    .setMessage("Bạn có chắc xóa Khoa này?")
                    .setPositiveButton("Có", (dialog, which) -> {
                        boolean deleted = khoaQuery.xoaKhoa(khoaEdit.getMaKhoa());
                        if (deleted) {
