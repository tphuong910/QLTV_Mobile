...
public class SQLiteHelper extends SQLiteOpenHelper {
    ...
    private static final int DB_VERSION = 3; // Tăng version để thêm cột User/Pass cho docgia
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion < 3) {
            // Thêm cột User và Pass cho bảng docgia để hỗ trợ đăng nhập khách hàng mà không làm mất dữ liệu cũ
            try {
                db.execSQL("ALTER TABLE docgia ADD COLUMN User TEXT");
                db.execSQL("ALTER TABLE docgia ADD COLUMN Pass TEXT");
    ...
    private void createTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS docgia (" +
                ...
                "Email TEXT NOT NULL," +
                "Sdt TEXT NOT NULL," +
                "User TEXT," + // Thêm cột User
                "Pass TEXT," + // Thêm cột Pass
                "FOREIGN KEY(MaKhoa) REFERENCES khoa(MaKhoa)," +
